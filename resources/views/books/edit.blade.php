        ...
        <div class="card w-full base-100 mt-5 mb-5 border border-gray-300">
            <div class="card-title bg-blue-500 text-white px-4 py-3 rounded-t-lg">
                <div class="flex items-center justify-between w-full">
                    <h2 class="text-bold text-xl text-white">Edit Book</h2>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('book.update', $book->id) }}" method="POST" class="form-group">
                    ...
                    <label class="input input-bordered bg-white flex items-center gap-2 mt-2">
                        ...
                        <input type="text" class="grow text-black" placeholder="Subject" name="subject" value="{{ $book->subject }}" required/>
                    </label>
                    <br />
                    <label for="isbn" class="text-black">ISBN:</label>
                    <label class="input input-bordered bg-white flex items-center gap-2 mt-2">
                        <x-fas-book class="w-4 h-4 opacity-70 text-black"/>
                        <input type="text" class="grow text-black" placeholder="ISBN" name="isbn" value="{{ $book->isbn }}" required/>
                    </label>
                    <br />
                    <label for="title" class="text-black">Title:</label>
                    <label class="input input-bordered bg-white flex items-center gap-2 mt-2">
                        <x-fas-font class="w-4 h-4 opacity-70 text-black"/>
                        <input type="text" class="grow text-black" placeholder="Title" name="title" value="{{ $book->title }}" required/>
                    </label>
                    ...
                    <label class="input input-bordered bg-white flex items-center gap-2 mt-2">
                        ...
                        <input type="date" class="w-full text-black" placeholder="Record Date" name="record_date" value="{{ $book->record_date }}" required/>
                    </label>
