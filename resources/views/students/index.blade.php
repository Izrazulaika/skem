<x-app-layout>
    ...
    <div class="container">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-bold text-2xl text-black">List of Students</h2>
            <a href="{{ route('students.create') }}" ><button class="h-fit min-h-fit py-2 px-4 btn btn-info text-white">Add</button></a>
        </div>
                ...
                @if (count($students) > 0)
                    ...
                    @foreach ($students as $student)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('students.edit', $student->id) }}" ><button class="h-fit min-h-fit py-2 px-4 btn btn-info text-white">Edit</button></a>
                                <form action="{{ route('students.destroy', $student->id) }}" method="POST" style="display: inline-block;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="h-fit min-h-fit py-2 px-4 btn btn-error text-white" onclick="return confirm('Are you sure you want to delete this student?')">Delete</button>
                                </form>
                            </td>
                        </tr>
                        {{-- @php $count++; @endphp --}}
