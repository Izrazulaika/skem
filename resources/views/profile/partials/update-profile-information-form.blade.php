<section>
    ...
    @if (session('success-profile'))
        <div role="alert" class="alert alert-success mt-3">
            ...
            <span class="text-white">{{ session('success-profile') }}</span>
    ...
    @if (session('error-profile'))
        <div role="alert" class="alert alert-error mt-3">
            ...
            <span class="text-white">{{ session('error-profile') }}</span>
            <button type="button" class="btn-clear float-right" aria-label="Close"><x-fas-circle-xmark class="w-4 h-4 text-white"/></button>
        </div>
    @endif
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        @method('patch')
        @php
            $phoneNumber = !empty($user->userDetail) ? $user->userDetail->phone_number : '';
        @endphp
        <div>
        ...
        <div>
            <x-input-label for="phone_number" class="text-black" :value="__('Mobile Number')" />
            <x-text-input id="phone_number" name="phone_number" type="text" class="mt-1 block w-full" :value="old('phone_number', $phoneNumber)" required autofocus autocomplete="name" />
        ...
        <div>
            ...
            <x-text-input id="role" name="role" type="text" class="mt-1 block w-full" :value="$user->role == 'admin' ? 'Admin': 'Parent'" autocomplete="role" disabled />
